...
{
    int len = strnlen(filename, RODOOS_MAX_PATH);
    return (len >= 3 && isdigit(filename[0]) && memcmp((void*)&filename[1], ":/", 2) == 0);
}

static int pathparser_get_driver_by_path(const char** path)
{
    ...
    int driver_no = to_numeric_digit(*path[0]);
    *path += 3;
    return driver_no;
...
{
    char* result_path_part  = kzalloc(RODOOS_MAX_PATH);
    int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    if (strlen(path) > RODOOS_MAX_PATH)
    {
        goto out;
    }
